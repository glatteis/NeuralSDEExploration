...
@testset "Exported Latent SDE" begin
    ...
    grads_interpolating = Zygote.gradient((x) -> loss(x, interpolating, (seed) -> nothing), ps)[1]
    grads_finitediff_2 = FiniteDiff.finite_difference_gradient((x) -> loss(x, nothing, (seed) -> nothing), ps) 
    ...
    @test length(outliers1) == 0
    @test length(outliers2) == 0
end
