...
export LatentSDEFlux, sample_prior

struct LatentSDEFlux{N1,N2,N3,N4,N5,N6,N7,R1,R2,R3,R4,R5,R6,R7,P1,P2,P3,P4,P5,P6,P7,T,A,K}
    initial_prior::N1
...
function LatentSDEFlux(initial_prior, initial_posterior, drift_prior, drift_posterior, diffusion, encoder, projector, tspan, args...; kwargs...)
...
function sample_prior(n::LatentSDEFlux, ps; b=1, seed=nothing)
    if seed !== nothing
        Random.seed!(seed)
    end
...
function sample_posterior(n::LatentSDEFlux, ps, timeseries; seed=nothing)
    if seed !== nothing
        Random.seed!(seed)
    end
...
function pass(n::LatentSDEFlux, ps::ComponentVector, timeseries; sense=InterpolatingAdjoint(autojacvec=ZygoteVJP()), ensemblemode=EnsembleSerial(), seed=nothing, noise=nothing)
    ...
    projected_ts = reduce(timecat, [n.projector_re(ps.projector_p)(x) for x in eachslice(posterior_latent, dims=3)])
...
function loss(n::LatentSDEFlux, ps, timeseries, beta; kwargs...)
