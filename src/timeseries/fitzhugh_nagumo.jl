...
function series(fhn::FitzHughNagumoModel, u0s, tspan, datasize; batchsize=20)
    t = range(tspan[1], tspan[2], length=datasize)
    [NODEDataloader(solve(SDEProblem(drift, diffusion, u0, tspan, fhn, reltol=0.01), saveat=t), batchsize) for u0 in u0s]
end
